...
<template>
  <header className="bg-white border-b shadow-sm border-gray-200 sticky top-0 z-50">
      <div className="mx-auto px-4 relative py-4 flex justify-between items-center">
        ...
        >
          Cart
            <span v-if="cartCount > 0" className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center">
              {{cartCount}}
            </span>
        </button>
        ...
        <form
        ref="formRef"
        v-if="isOpenLogin" class="absolute right-5 shadow-xl rounded-xl top-20 bg-white p-5 flex flex-col gap-5">
          ...
          <ButtonAction class="mt-5" type="button" button-name="Login" :onClick="onSubmit" :disabled="isSubmitting" :is-loading="isSubmitting" />
        </form>
        </div>
      </div>
    </header>
